...
<body>
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('saveUpdateTicket', $package->id) }}">
                            @csrf
                            @method('patch')
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td><input type="text" name="name" class="form-control"
                                                value="{{  $package->name }}" required></td>
                                        <td><input type="text" name="short_code" class="form-control"
                                                value="{{ $package->short_code }}" required></td>
                                        <td>
                                            @php
                                                $selectedEvents = json_decode($package->events_included, true) ?? [];
                                            @endphp
                                        ...
                                        <td><input type="text" name="price" class="form-control"
                                                value="{{  $package->price }}" required></td>
                                        <td><input type="text" name="description" class="form-control"
                                                value="{{  $package->description }}" required></td>
                                        ...
                                        <td>
                                            <div class="form-check form-switch">
                                                ...
                                                <input class="form-check-input" type="checkbox" name="visibility"
                                                    value="1" {{ $package->visibility ? 'checked' : '' }}>
                                            </div>
                                        </td>
    ...
    <script>
        $(document).ready(function () {
            $('.select2').select2({
                tags: true, // allows adding new text
                tokenSeparators: [',', ' '] // typing comma or space creates a new tag
            });
        });
    </script>
</body>
